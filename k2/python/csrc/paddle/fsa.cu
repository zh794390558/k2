...
#include "k2/python/csrc/paddle/v2/ragged_any.h"
...
static void PybindFsaUtil(py::module &m) {
  ...
  m.def(
      "fsa_to_tensor",
      [](const FsaOrVec &fsa) -> paddle::Tensor {
        DeviceGuard guard(fsa.Context());
        if (fsa.NumAxes() == 2) {
          Tensor tensor = FsaToTensor(fsa);
          return ToPaddle(tensor);
  ...
  m.def(  
      ...
      [](Fsa &fsa, bool openfst = false,
         std::vector<paddle::Tensor> extra_labels = std::vector<paddle::Tensor>(),
         paddle::optional<std::vector<RaggedAny>> ragged = {}) -> std::string {
        std::vector<Ragged<int32_t>> ragged_labels;
        if (ragged) {
          ragged_labels.reserve(ragged.get().size());
          for (const auto &r : ragged.get()) {
        ...
        for (size_t i = 0; i < extra_labels.size(); i++) {
          extra_labels_arrays[i] = FromPaddle<int32_t>(extra_labels[i]);
        }
  ...
  m.def(
      ...
      [](const std::string &s, int num_extra_labels = 0,
         ...
         bool openfst =
             false) -> std::tuple<Fsa, paddle::optional<paddle::Tensor>,
        ...
        if (num_extra_labels != 0) tensor = ToPaddle(extra_labels);
      ...
      " is 0, else a 2-D tensor of dtype paddle.int32 and shape "
      "(num_extra_labels, num_arcs) if num_extra_labels > 0; otherwise None; "
      "`ragged_labels` is a list of RaggedAny (dtype is paddle.int32) of length "
      "`num_ragged_labels`");
  ...
  // returns RaggedAny with dtype paddle.int32
  m.def(
      "get_state_batches",
  ...
  m.def(
      "get_dest_states",
      [](FsaVec &fsas, bool as_idx01) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
  ...
  m.def(
      "get_incoming_arcs",
      [](FsaVec &fsas, paddle::Tensor dest_states) -> RaggedAny {
        DeviceGuard guard(fsas.Context());
        Array1<int32_t> dest_states_array = FromPaddle<int32_t>(dest_states);
...
static void PybindGetForwardScores(py::module &m, const char *name) {
  ...
  m.def(
      ...
      [](FsaVec &fsas, RaggedAny &state_batches,
        ...
        paddle::optional<paddle::Tensor> entering_arcs_tensor;
        if (!log_semiring) entering_arcs_tensor = ToPaddle(entering_arcs);

        return std::make_pair(ToPaddle(scores), entering_arcs_tensor);
      },
      py::arg("fsas"), py::arg("state_batches"),
...
static void PybindBackpropGetForwardScores(py::module &m, const char *name) {
  ...
  m.def(
      ...
      [](FsaVec &fsas, RaggedAny &state_batches, RaggedAny &leaving_arc_batches,
         bool log_semiring, paddle::optional<paddle::Tensor> entering_arcs,
         paddle::Tensor forward_scores,
         paddle::Tensor forward_scores_deriv) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
        Array1<T> forward_scores_array = FromPaddle<T>(forward_scores);
        Array1<T> forward_scores_deriv_array =
            FromPaddle<T>(forward_scores_deriv);
        Array1<int32_t> entering_arcs_array;
        const Array1<int32_t> *p_entering_arcs = nullptr;

        if (!log_semiring) {
          K2_CHECK(entering_arcs)
              << "You have to provide entering_arcs for tropical semiring";
          entering_arcs_array = FromPaddle<int32_t>(*entering_arcs);
...
static void PybindGetBackwardScores(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaVec &fsas, RaggedAny &state_batches, RaggedAny &leaving_arc_batches,
         bool log_semiring = true) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
...
static void PybindBackpropGetBackwardScores(py::module &m, const char *name) {
  m.def(
      ...
      [](FsaVec &fsas, RaggedAny &state_batches,
         RaggedAny &entering_arc_batches, bool log_semiring,
         paddle::Tensor backward_scores,
         paddle::Tensor backward_scores_deriv) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
        Array1<T> backward_scores_array = FromPaddle<T>(backward_scores);
        Array1<T> backward_scores_deriv_array =
            FromPaddle<T>(backward_scores_deriv);
...
static void PybindGetTotScores(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaVec &fsas, paddle::Tensor forward_scores) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
        Array1<T> forward_scores_array = FromPaddle<T>(forward_scores);
        Array1<T> tot_scores = GetTotScores(fsas, forward_scores_array);
        return ToPaddle(tot_scores);
      },
...
static void PybindDenseFsaVec(py::module &m) {
  ...
  pyclass.def(
      py::init([](paddle::Tensor scores,
                  paddle::Tensor row_splits) -> std::unique_ptr<DenseFsaVec> {
        ...
        K2_CHECK(scores.is_dense_tensor());
        Array1<int32_t> row_splits_array = FromPaddle<int32_t>(row_splits);

        RaggedShape shape = RaggedShape2(&row_splits_array, nullptr, -1);
        Array2<float> scores_array = FromPaddle<float>(scores, Array2Tag{});
...
static void PybindGetArcPost(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaVec &fsas, paddle::Tensor forward_scores,
         paddle::Tensor backward_scores) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
        Array1<T> forward_scores_array = FromPaddle<T>(forward_scores);
        Array1<T> backward_scores_array = FromPaddle<T>(backward_scores);
        Array1<T> arc_post =
            GetArcPost<T>(fsas, forward_scores_array, backward_scores_array);
        return ToPaddle(arc_post);
      },
...
static void PybindBackpropGetArcPost(py::module &m, const char *name) {
  ...
  m.def(
      name,
      [](FsaVec &fsas, RaggedAny &incoming_arcs, paddle::Tensor arc_post_deriv)
          -> std::pair<paddle::Tensor, paddle::Tensor> {
        DeviceGuard guard(fsas.Context());
        Array1<T> arc_post_deriv_array = FromPaddle<T>(arc_post_deriv);
        ...
        BackpropGetArcPost<T>(fsas, incoming_arcs.any.Specialize<int32_t>(),
                              ...
                              &backward_scores_deriv);
        return std::make_pair(ToPaddle(forward_scores_deriv),
                              ToPaddle(backward_scores_deriv));
      },
 ...
 */
template <typename T>
/*static*/ paddle::Tensor GetTotScoresTropicalBackward(
    FsaVec &fsas, const RaggedAny &best_path_arc_indexes,
    paddle::Tensor tot_scores_grad) {
  DeviceGuard guard(fsas.Context());
  ...
  K2_CHECK_EQ(tot_scores_grad.shape()[0], static_cast<int64_t>(num_fsas));
  // K2_CHECK_EQ(tot_scores_grad.dim(), 1);
  K2_CHECK_EQ(tot_scores_grad.dims().size(), 1);
  // K2_CHECK_EQ(tot_scores_grad.scalar_type(), ToScalarType<T>::value);
  K2_CHECK_EQ(tot_scores_grad.dtype(), ToScalarType<T>::value);
  ...
  paddle::Tensor ans_grad = paddle::zeros(dims, paddle::DataType::FLOAT32, tot_scores_grad.place());
  // float *ans_grad_data = ans_grad.data_ptr<float>();
  float *ans_grad_data = ans_grad.data<float>();

  // const T *tot_scores_grad_data = tot_scores_grad.data_ptr<T>();
  const T *tot_scores_grad_data = tot_scores_grad.data<T>();
  // int64_t tot_scores_grad_stride = tot_scores_grad.strides()[0];
  phi::IntArray strides = phi::vectorize(phi::stride(tot_scores_grad.dims()));
  int64_t tot_scores_grad_stride = strides[0];
 ...
 */
template <typename T>
/*static*/ paddle::Tensor GetTotScoresLogBackward(
    FsaVec &fsas, paddle::Tensor arc_post, paddle::Tensor tot_scores_grad) {
  DeviceGuard guard(fsas.Context());
  K2_CHECK_EQ(fsas.NumAxes(), 3);
  K2_CHECK_EQ(fsas.NumElements(), arc_post.numel());
  // K2_CHECK(arc_post.is_contiguous())
  K2_CHECK(arc_post.is_dense_tensor())
      << "arc_post is supposed to be computed by k2 "
         "so it should be contiguous!";
  // K2_CHECK_EQ(arc_post.dim(), 1);
  K2_CHECK_EQ(arc_post.dims().size(), 1);
  // K2_CHECK_EQ(arc_post.scalar_type(), ToScalarType<T>::value);
   K2_CHECK_EQ(arc_post.dtype(), ToScalarType<T>::value);
  // K2_CHECK_EQ(tot_scores_grad.dim(), 1);
  K2_CHECK_EQ(tot_scores_grad.dims().size(), 1);
  // K2_CHECK_EQ(tot_scores_grad.sizes()[0], static_cast<int64_t>(fsas.Dim0()));
  K2_CHECK_EQ(tot_scores_grad.shape()[0], static_cast<int64_t>(fsas.Dim0()));
  // K2_CHECK_EQ(tot_scores_grad.scalar_type(), ToScalarType<T>::value);
  K2_CHECK_EQ(tot_scores_grad.dtype(), ToScalarType<T>::value);
  ...
  paddle::Tensor ans_grad = paddle::empty(dims, paddle::DataType::FLOAT32, tot_scores_grad.place());
  // float *ans_grad_data = ans_grad.data_ptr<float>();
  float *ans_grad_data = ans_grad.data<float>();
  // const T *tot_scores_grad_data = tot_scores_grad.data_ptr<T>();
  const T *tot_scores_grad_data = tot_scores_grad.data<T>();
  ...
  const T *arc_post_data = arc_post.data<T>();
...
static void PybindGetArcCdf(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaOrVec &fsas, paddle::Tensor arc_post) -> paddle::Tensor {
        DeviceGuard guard(fsas.Context());
        Array1<T> arc_post_array = FromPaddle<T>(arc_post);
...
static void PybindRandomPaths(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaVec &fsas, paddle::Tensor arc_cdf, int32_t num_paths,
         paddle::Tensor tot_scores, RaggedAny &state_batches) -> RaggedAny {
        DeviceGuard guard(fsas.Context());
        Array1<T> arc_cdf_array = FromPaddle<T>(arc_cdf);
        Array1<T> tot_scores_array = FromPaddle<T>(tot_scores);
...
static void PybindPruneOnArcPost(py::module &m, const char *name) {
  m.def(
      name,
      [](FsaVec &fsas, paddle::Tensor arc_post, T threshold_prob,
         bool need_arc_map =
             true) -> std::pair<FsaVec, paddle::optional<paddle::Tensor>> {
        DeviceGuard guard(fsas.Context());
        Array1<T> arc_post_array = FromPaddle<T>(arc_post);
        Array1<int32_t> arc_map;
        FsaVec ans = PruneOnArcPost(fsas, arc_post_array, threshold_prob,
                                    need_arc_map ? &arc_map : nullptr);
        paddle::optional<paddle::Tensor> arc_map_tensor;
        if (need_arc_map) arc_map_tensor = ToPaddle(arc_map);
