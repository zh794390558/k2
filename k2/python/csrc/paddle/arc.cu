...
static void PybindArcImpl(py::module &m) {
  ...
  m.def(
      "as_int",
      [](paddle::Tensor tensor) -> paddle::Tensor {
        ...
        if (tensor.numel() == 0)
          ...
          return paddle::empty(tensor.shape(), scalar_type, tensor.place());
        ...
        return paddle::from_blob(
            tensor.data(), tensor.shape(), scalar_type, tensor.layout(), tensor.place(),
            [tensor](void *p){});
      },
      py::arg("tensor"));
