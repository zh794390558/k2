...
#include "k2/python/csrc/paddle/v2/autograd/index_and_sum.h"
#include "k2/python/csrc/paddle/v2/autograd/logsumexp.h"
#include "k2/python/csrc/paddle/v2/autograd/normalize.h"
#include "k2/python/csrc/paddle/v2/autograd/sum.h"
...
RaggedAny::RaggedAny(const RaggedShape &shape, paddle::Tensor value)
    : data(value) {
...
RaggedAny::RaggedAny(const std::string &s, py::object dtype /*=py::none()*/,
                     paddle::Device device /*=paddle::kCPU*/) {
  if (!dtype.is_none() && !THPDtype_Check(dtype.ptr())) {
    K2_LOG(FATAL) << "Expect an instance of paddle.dtype. "
                  << "Given: " << py::str(dtype);
  }
...
RaggedAny::RaggedAny(paddle::Tensor tensor) {
...
const paddle::Tensor &RaggedAny::Data() const {
  DeviceGuard guard(any.Context());
  if (!data.defined()) {
    Dtype t = any.GetDtype();
    FOR_REAL_AND_INT32_TYPES(t, T, {
      const_cast<RaggedAny *>(this)->data =
          ToPaddle((const_cast<RaggedAny *>(this)->any).Specialize<T>().values);
    });
  }
  return data;
}
...
RaggedAny RaggedAny::To(paddle::Device device) const {
  ContextPtr context = any.Context();
  if (device.is_cpu()) {
    // CPU -> CPU
...
RaggedAny RaggedAny::To(const std::string &device) const {
  paddle::Device d(device);
  return this->To(d);
}

RaggedAny RaggedAny::To(paddle::ScalarType scalar_type) const {
...
RaggedAny &RaggedAny::SetRequiresGrad(bool requires_grad /*=true*/) {
  // PyTorch will throw a RuntimeError exception if dtype is paddle.int32
  // So no need to check it by us here
  // Data().requires_grad_(requires_grad);
  Data().mutable_autograd_meta()->SetStopGradient(!requires_grad);
  return *this;
}

paddle::Tensor RaggedAny::Sum(float initial_value /*=0*/) const {
  DeviceGuard guard(any.Context());
  return SumFunction::apply(*this, Data(), initial_value);
}

paddle::Tensor RaggedAny::LogSumExp(
...
paddle::Tensor RaggedAny::ArgMax(
  ...
  return ToPaddle(indexes);
}

paddle::Tensor RaggedAny::Max(
    py::object initial_value /*=py::none()*/) /*const*/ {
  K2_CHECK((bool)initial_value);
  ...
  FOR_REAL_AND_INT32_TYPES(t, T, {
    ...
    return ToPaddle(max_values);
  });
  // Unreachable code
  return {};
}

paddle::Tensor RaggedAny::Min(
    py::object initial_value /*=py::none()*/) /*const*/ {
  K2_CHECK((bool)initial_value);
...
RaggedAny::Unique(bool need_num_repeats /*= false*/,
  ...
  paddle::optional<RaggedAny> num_repeats_tensor;
  if (need_num_repeats) num_repeats_tensor = RaggedAny(num_repeats.Generic());

  paddle::optional<paddle::Tensor> new2old_indexes_tensor;
  if (need_new2old_indexes) new2old_indexes_tensor = ToPaddle(new2old_indexes);
...
RaggedAny RaggedAny::Add(paddle::Tensor value, py::object alpha) /*const*/ {
  K2_CHECK((bool)alpha);
...
paddle::Tensor RaggedAny::Pad(const std::string &mode,
...
paddle::optional<paddle::Tensor> RaggedAny::Sort(
  ...
  paddle::optional<paddle::Tensor> ans;
  if (need_new2old_indexes) ans = ToPaddle(new2old);
  return ans;
}
...
std::pair<RaggedAny, paddle::optional<paddle::Tensor>> RaggedAny::Index(
  ...
  paddle::optional<paddle::Tensor> value_indexes_tensor;
  ...
  FOR_REAL_AND_INT32_TYPES(t, T, {
    ...
    if (need_value_indexes) value_indexes_tensor = ToPaddle(value_indexes);
...
RaggedAny RaggedAny::Index(paddle::Tensor src,
...
paddle::Tensor RaggedAny::IndexAndSum(paddle::Tensor src) /*const*/ {
  DeviceGuard guard(any.Context());
